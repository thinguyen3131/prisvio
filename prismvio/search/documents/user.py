...
class UsersDocument(Document):
    ...
    privacy_settings = fields.NestedField(
        properties={
...
class UserSearch(Search):
    ...
    def custom_search(self, queryset, request: UserSearchRequest):
        ...
        privacy_conditions_user_name = Nested(
            path="privacy_settings", query=Q("term", privacy_settings__username_privacy=PrivacySetting.EVERYONE)
        )
        privacy_conditions_email = Nested(
            path="privacy_settings", query=Q("term", privacy_settings__email_privacy=PrivacySetting.EVERYONE)
        )
        ...
        if request.friend_ids:
            friends_username_conditions = Nested(
                path="privacy_settings", query=Q("term", privacy_settings__username_privacy=PrivacySetting.FRIENDS)
            )

            friends_email_conditions = Nested(
                path="privacy_settings", query=Q("term", privacy_settings__email_privacy=PrivacySetting.FRIENDS)
            )
            friends_phone_number_conditions = Nested(
                path="privacy_settings", query=Q("term", privacy_settings__phone_number_privacy=PrivacySetting.FRIENDS)
            )
