...
__all__ = [
    ...
    "generate_otp",
    "is_valid_otp",
    "is_valid_otp_instance",
    "is_valid_otp_and_signature",
    "get_otp_instance",
    "require_new_otp",
]
...
def get_secret_key(signature):
    user_key = f"{signature}_{settings.SECRET_KEY}"
...
def generate_otp(signature=None, interval=INTERVAL_TIME, for_time=None):
    ...
    log(f"generate_otp: Code={code}, sig={signature}, input={for_time}")
    return code
...
def is_valid_otp(otp, signature=None, interval=INTERVAL_TIME, for_time=None):
    ...
    log(f"is_valid_otp: Rs={result}, Code={otp}, S={signature}, Key={key}, Interval={interval} \n")
    return result
...
def get_otp_instance(signature, otp_type=None, otp_action=None):
    try:
