...
from prismvio.users.enums import IntervalLockTime, MaxTime, OTPAction, OTPType
...
def generate_signature(*args):
    ...
    for item in args:
        text += str(item)
    signature = f"{text}{settings.SECRET_KEY}{timezone.now()}"
...
class OneTimePassword(models.Model):
    user = models.ForeignKey(User, related_name="otp_set", null=True, blank=True, on_delete=models.CASCADE)
    email = models.EmailField(null=True, blank=True, db_index=True)
    phone_number = models.CharField(max_length=45, null=True, blank=True, db_index=True)
