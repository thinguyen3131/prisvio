...
from menuMerchant.models import Products, Promotion, Services
from booking.enums import BookingStatusEnum
from booking.models import Booking, BookingProduct, BookingPromotion, BookingService
from booking.services.push import PushBookingMessage
from booking.tasks import create_booking_event_task
...
class ServiceSerializer(serializers.ModelSerializer):
    class Meta:
        model = Services
        exclude = ('hashtags', 'merchant', 'owner', 'category', 'original_price', 'discount_price', 'time', 'time_date')
...
class BookingServiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'service', 'service_id', 'staff',  'duration', 'price',
                  'quantity', 'note', 'start_date', 'end_date', 'is_anyone', 'service_info',
                  'staff_info', 'merchant_info')
        extra_kwargs = {
...
class BookingProductSerializer(serializers.ModelSerializer):
    ...
    product_id = serializers.PrimaryKeyRelatedField(write_only=True, source='products', queryset=Products.objects.all())
...
class BookingPromotionSerializer(serializers.ModelSerializer):
    ...
    promotion_id = serializers.PrimaryKeyRelatedField(write_only=True,
                                                      source='promotion',
    ...
    def create(self, validated_data):
        ...
        if products:
            ...
            for data in products:
                ...
                # update product
                product.quantity = product.quantity - data.get('quantity', 0)
