...
class IsBusinessAdminOrAdmin(BasePermission):
    """
    This permission allows users with 'business_admin' or 'is_staff' (is_admin) permission to make all changes (CREATE, UPDATE, DELETE).
     Users other than 'business_admin' or 'is_staff' (is_admin) only have permission to view the object (READ).
    """
    def has_permission(self, request, view):
        # Allow SAFE_METHODS (GET, HEAD, OPTIONS) methods for all users
        if request.method in SAFE_METHODS:
