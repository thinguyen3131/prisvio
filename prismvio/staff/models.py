...
from prismvio.staff.enums import GenderEnum, InviteStatusEnum, LinkStatusEnum
...
class Staff(models.Model):
    ...
    invite_status = models.CharField(
        max_length=50, null=True, blank=True, choices=InviteStatusEnum.choices, default=None
    )
    link_status = models.CharField(
        max_length=50, null=True, blank=True, choices=LinkStatusEnum.choices, default=LinkStatusEnum.UNLINKED
    )

    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True)
    merchant = models.ForeignKey(
        "merchant.Merchant", related_name="staff", null=True, blank=True, on_delete=models.SET_NULL
    )
